...
const Home = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      if (!isAnimating) setLoading(true);
      try {
        const response = await axios.get('http://localhost:5000/api/products', {
          params: { page: currentPage, keyword, category, minPrice, maxPrice }
        });
        if (response.data.success) {
          const newProducts = response.data.products || [];
          const isPageChange = currentPage !== prevPageRef.current;
          
          if (isPageChange && products.length > 0) {
            const direction = currentPage > prevPageRef.current ? 'right' : 'left';
        ...
        if (!isAnimating) setLoading(false);
      }
  ...
  const renderProductGrid = (gridData, isLoading) => (
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 px-4 w-full flex-shrink-0 transition-opacity duration-300 ${isLoading ? 'opacity-50' : 'opacity-100'}`}>
      {gridData.length === 0 && !isLoading ? (
        ...
        gridData.map((sach, index) => (
          <div key={sach._id} className="manga-card-tilt bg-white dark:bg-gray-800 border-4 border-black dark:border-white shadow-comic dark:shadow-comic-dark hover:shadow-comic-lg dark:hover:shadow-comic-lg-dark transition-all flex flex-col group relative overflow-hidden transform animate-pulse-shadow">
            {sach.stock <= 0 && (
  ...
  return (
    <div className="w-full">
      ...
      <div id="gridWrapper" className="relative overflow-hidden w-full flex flex-nowrap" style={{ minHeight: '800px' }}>
          {isAnimating && slideDirection === 'left' && (
              <div className={`w-full flex-shrink-0 transition-transform duration-500 ease-in-out ${animationPhase === 'start' ? '-translate-x-full' : 'translate-x-0'}`}>
                  {renderProductGrid(products, false)}
              </div>
          )}

          <div className={`w-full flex-shrink-0 transition-transform duration-500 ease-in-out ${!isAnimating ? 'translate-x-0' : (slideDirection === 'right' ? (animationPhase === 'start' ? 'translate-x-0' : '-translate-x-full') : (animationPhase === 'start' ? '-translate-x-full' : 'translate-x-0'))}`}>
              {renderProductGrid(isAnimating ? prevProducts : products, loading && !isAnimating)}
          </div>

          {isAnimating && slideDirection === 'right' && (
              <div className={`w-full flex-shrink-0 transition-transform duration-500 ease-in-out ${animationPhase === 'start' ? 'translate-x-0' : '-translate-x-full'}`}>
                  {renderProductGrid(products, false)}
              </div>
          )}
      </div>
